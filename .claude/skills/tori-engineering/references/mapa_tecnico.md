...
10. Los exports con foto usan `_fotoThumbForExport` y anclaje EMU (`_aOff=18000`, `editAs:'twoCell'`). Desde v5_101 los exports DE PROVEEDOR (Excel FOTOS de la pestaña China y `addChinaSheet` — Distribuidor + pedidos 🏭) van NÍTIDOS: `(foto, 1200, 0.8)` = la foto de la bóveda pasa TAL CUAL (el helper descarta el re-encode si sale más pesado), extensión detectada png/jpeg, `_aImg=914400` (96px) y row height 76. Los exports internos (Traslado San Benito, Repetidos) siguen en `(480, 0.78)` / `_aImg=609600` / height 52. OJO: las fotos de las facturas reales de proveedor vienen en ~203px nativos — TORI ya conserva TODO lo que llega; más nitidez requiere que el proveedor mande fotos más grandes (la tubería aguanta hasta 1200px).
